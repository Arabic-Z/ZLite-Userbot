...
@Client.on_message(filters.command(["اذاعه", "اذاعة", "chatbroadcast"], [".", "!", "."]) & filters.user(SUDO_USER))
async def chat_broadcast(c: Client, m: Message):
    if m.reply_to_message:
        msg = m.reply_to_message.text.markdown
    else:
        await m.reply_text("**- بالــرد ع رسـالـه . . لـ الاذاعــه**")
        return

    exmsg = await m.reply_text("**- جــاري الاذاعــه . . .**")
    ...
    async for dialog in c.iter_dialogs():
          try:
            ...
            await m.reply_text(f"[اذاعــه] {dialog.chat.id} {e}")
...
@Client.on_message(filters.command('مغادره', [".", "!"]) & filters.user(SUDO_USER))
