...
@Client.on_message(filters.command(["وقت التشغيل", "up"], ".") & filters.me)
async def uptime(client: Client, message: Message):
    now = datetime.now()
    current_uptime = now - START_TIME
    await message.edit(f"**- وقـت تشغيـل البـوت الفعـلي**\n\n" f"```{str(current_uptime).split('.')[0]}```")


@Client.on_message(filters.command("iid", ".") & filters.me)
