...
from pyrogram.errors import FloodWait, MessageNotModified, UserAdminInvalid
...
async def member_ban(client: Client, message: Message):
    if message.chat.type in ["group", "supergroup"]:
        ...
        if not me_.can_restrict_members:
         await message.edit("**- عـذراً . . ليـس لـديك صـلاحيـات هنـا**")
         return
        can_ban= True
        if can_ban:
            try:
            ...
            if user_id:
                try:
                    ...
                    await message.edit_text("**- عـذراً . . ليـس لـديك صـلاحيـات هنـا**")
                    return
...
@Client.on_message(filters.command("طرد", ".") & filters.me)
async def kick_user(bot: Client, message: Message):
    if await CheckReplyAdmin(message) and await CheckAdmin(message):
        try:
            ...
            await message.edit(f"**- بيبــاي** , {mention}.")
        except UserAdminInvalid:
            await RestrictFailed(message)


#العـام
@Client.on_message(filters.me & filters.command("كتم عام", ["."]))
...
@Client.on_message(filters.me & filters.command("الغاء الكتم العام", ["."]))
async def gmute_him(client: Client, message: Message):
...
@Client.on_message(filters.me & filters.command("حظر عام", ["."]))
...
@Client.on_message(filters.me & filters.command("الغاء الحظر العام", ["."]))
...
@Client.on_message(filters.me & filters.command("المحظورين عام", ["."]))
...
add_command_help(
    "الادمن2",
    [
        [".حظر", "Bans user for specified hours or indefinitely."],
        [".الغاء حظر", "Unbans the user."],
        [".كتم", "Bans user for specified hours or indefinitely."],
        [".الغاء كتم", "Unmutes the user."],
        [".طرد", "Kicks the user out of the group."],
    ],
)
