...
from helpers.interval import IntervalHelper
from main import *
...
async def CheckReplyAdmin(message: Message):
    ...
    if not message.reply_to_message:
        ...
        await message.edit(f"**- لا استطـيع** {message.command[0]} **نفسـي . . .**")
        await asyncio.sleep(2)
        await message.delete()
    else:
        return True
...
async def RestrictFailed(message: Message):
    await message.edit(f"**- عـذراً .. لا استطيـع** {message.command} **هـذا الشخـص**")
    await asyncio.sleep(2)
    await message.delete()
